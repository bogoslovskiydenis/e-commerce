...
15. [Что уже сделано](#что-уже-сделано)
16. [Что осталось сделать](#что-осталось-сделать)
...
- Восстановление пароля (см. раздел «Что осталось сделать»)
- Управление личным кабинетом
- История заказов

**Текущий статус:** Витрина использует `AuthContext` и `apiService`: регистрация и вход через `POST /api/customers/auth/register`, `POST /api/customers/auth/login`, обновление токена `POST /api/customers/auth/refresh`, профиль `GET /api/customers/auth/me`, заказы клиента `GET /api/customers/auth/orders`, обновление профиля `PUT /api/customers/auth/profile`. Страницы `/forgot-password` и `/reset-password` есть в UI, но вызовы API восстановления пароля на фронте и соответствующие эндпоинты в `api` пока не связаны.
...
- Vuetify 3
- Pinia
- SSR отключён (`ssr: false` в `nuxt.config`)
...
- **Клиентский магазин (`ui-e-commerce`)**: Next.js (App Router, TypeScript, Tailwind), порт 3010.
- **Админ‑панель (`admin-panel-nuxt`)**: Nuxt 3 + Vuetify + Pinia, порт 3000, перенесён функционал управления из старой React‑админки.
- **API‑сервис (`api`)**: NestJS + Prisma + PostgreSQL, порт 3001, единый REST‑backend для витрины и админки.

### Фронтенд (витрина)

- **Реализованные страницы**: главная (`/`), поиск (`/search`), категории и подкатегории (`/[category]`, `/[category]/[subcategory]`), карточка товара (`/product/[id]`), корзина (`/cart`), оформление заказа (`/checkout`, `/checkout/success`), избранное (`/favorites`), личный кабинет (`/account`), аутентификация покупателей (`/login`, `/forgot-password`, `/reset-password`), контентные страницы «О нас», «Контакты», «Доставка» (`/about`, `/contacts`, `/delivery`), распродажа (`/sale`).
- **Рабочие сценарии**: каталог с фильтрами и сортировками, детальная карточка товара с атрибутами и отзывами, корзина с подсчётом стоимости, многошаговый checkout, создание заказа, генерация PDF‑документов по заказу и запись аналитических событий.
- **Локализация витрины**: переключатель языка (uk, ru, en), строки в `src/locales`, запросы к API с учётом языка где предусмотрено (категории, страницы по slug и др.).
- **Статус логина покупателей**: регистрация, вход, refresh, профиль и список заказов клиента подключены к Customer API; восстановление пароля только как макет страниц без рабочего API (см. ниже).

### Админ‑панель

- **Разделы админки**: товары, категории, заказы, клиенты, отзывы и комментарии, обратные звонки, промокоды/акции, баннеры, навигация, страницы сайта, настройки сайта, аналитика, пользователи админки, API‑ключи, логи и системная информация.
- **Права и роли**: настроены роли `SUPER_ADMIN`, `ADMINISTRATOR`, `MANAGER`, `CRM_MANAGER`, детализированные permissions (например, `products.*`, `orders.*`, `website.*`, `analytics.*`) и их проверка как на уровне API, так и на уровне интерфейса админки.
- **CRUD и таблицы**: для основных сущностей (товары, заказы, клиенты, контент сайта) реализованы списки с фильтрами, сортировками, пагинацией и формами создания/редактирования в соответствии с разделами этой документации.

### API / Бэкенд

- **Модули**: в `AppModule` подключены `auth`, `users`, `products`, `categories`, `orders`, `customers`, `reviews`, `promotions`, `banners`, `callbacks`, `favorites`, `navigation`, `pages`, `settings`, `analytics`, `admin-logs`, `prisma` и общие `common`‑компоненты. Код модуля `comments` (`CommentsModule`, контроллер `/comments`) есть в репозитории, но модуль не импортирован в `AppModule`, поэтому REST для комментариев из админки (`admin-panel-nuxt` вызывает список `comments`) не обслуживается, пока не добавить импорт.
- **Функционал**: реализованы основные REST‑эндпоинты для каталога товаров, заказов, клиентов, отзывов, обратных звонков, избранного, навигации, статических страниц, настроек сайта и аналитики; настроена генерация PDF по заказам и раздача загруженных файлов через `/uploads`.
- **Безопасность и доступ**: работает JWT‑аутентификация, refresh‑токены, двухфакторная аутентификация, ролевая модель и система permissions, ведётся логирование действий администраторов.

### Общие вещи

- **Единая доменная модель**: товары, категории, заказы, клиенты, отзывы, баннеры, навигация, статические страницы, настройки сайта, промокоды, избранное, callbacks и события аналитики используются как во фронтовой витрине, так и в админке через одно и то же API.
- **Система ролей и прав**: согласованная конфигурация ролей и разрешений на уровне БД, API и клиентских приложений (см. раздел «Система ролей и разрешений»).
- **Интеграции аналитики**: в настройках сайта предусмотрены поля для Google Analytics, Google Tag Manager, Facebook Pixel и Яндекс.Метрики, а бэкенд хранит события аналитики заказов и поведения пользователей.

## Что осталось сделать

1. **Восстановление пароля покупателя** — реализовать на бэкенде выдачу токена/кода и смену пароля, добавить методы в `ui-e-commerce/src/services/api.ts` и связать с `/forgot-password` и `/reset-password` (сейчас страницы вызывают несуществующие методы сервиса).
2. **Comments API** — импортировать `CommentsModule` в `api/src/app.module.ts`, чтобы админский раздел комментариев и публичные маршруты контроллера соответствовали запущенному приложению; при необходимости проверить согласование полей с тем, что ожидает `admin-panel-nuxt`.
3. **Схема БД** — при использовании `api/schema.sql` как эталона держать его в соответствии с `schema.prisma` и миграциями Prisma (файл может расходиться с фактической моделью).
4. **Документация vs маршруты админки** — в тексте выше местами указаны пути вида `/products/:id/show`; в Nuxt‑админке детальный просмотр/редактирование завязаны на `pages/products/[id].vue` и `edit/[id].vue`; при правках документации сверять с фактической структурой `admin-panel-nuxt/pages`.
