...
- **Реализованные страницы**: главная (`/`), поиск (`/search`), категории и подкатегории (`/[category]`, `/[category]/[subcategory]`), карточка товара (`/product/[id]`), корзина (`/cart`), оформление заказа (`/checkout`, `/checkout/success`), избранное (`/favorites`), личный кабинет (`/account`), аутентификация покупателей (`/login`, `/forgot-password`, `/reset-password`).
...
- **Статус логина покупателей**: фронтовые формы реализованы, интеграция с API для клиентов (Customer auth) частично, как описано в разделе «Система аутентификации».
...
- **Модули**: доступны модули `auth`, `users`, `products`, `categories`, `orders`, `customers`, `reviews`, `comments`, `promotions`, `banners`, `callbacks`, `favorites`, `navigation`, `pages`, `settings`, `analytics`, `admin-logs`, `prisma` и общие `common`‑компоненты (guards, декораторы, фильтры исключений, логирующие интерцепторы).
